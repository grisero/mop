...
Once initiated, the owner of the instance becomes the administrator. An instance can host one or several **projects**. Each project can set **goals** and each goals can be divided in **tasks**. 

Every goal and task has a **status** associated with it. The next status come predefined:

- **planned**: Initial status. Planned to be made in the future.
- **working**: The team started working on it.
- **blocked**: Cannot continue due to another task/goal blocking its progress.
- **unblocked**: All blocking tasks have been achieved
- **approving**: Awaiting for approval.
- **achieved**: Approved and therefore achieved.
...
- **plan**: Can create goals or tasks, assign users to them and set tags.
